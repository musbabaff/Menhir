...
public class TimeoutMessageItem extends BlockMenuItem {
    public TimeoutMessageItem(BlockMenu<?> editMenu) {
        super(editMenu);
    }
    ...
    public void click(ItemClickEvent<MenhirBlock> itemClickEvent) {
        ...
        Colors.send(player, "#2C74B3Enter new respawn message into chat. You can use \\n to indicate new line. Enter none to remove the message");
        block.getPlugin().getEditValuesListener().awaitChatInput(player)
                ...
                .thenAccept(s -> Gui.runSync(() -> {
                    ...
                    if ("none".equalsIgnoreCase(s)) {
                        getState().getCoolDown().setRespawnMessage(null);
                    } else {
                        getState().getCoolDown().setRespawnMessage(s.replace("\\n", "\n"));
                    }
